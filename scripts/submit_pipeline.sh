...
case $PIPELINE in
    ...
    dna_seq)
        ...
        ;;
    rna_seq)
        PIPELINE_DIR="pipelines/rna_seq"
        ;;
    scrna_seq)
        PIPELINE_DIR="pipelines/scrna_seq"
        ;;
    methylation)
        PIPELINE_DIR="pipelines/methylation"
        ;;
    hic)
        PIPELINE_DIR="pipelines/hic"
        ;;
    long_read)
        PIPELINE_DIR="pipelines/long_read"
        ;;
    metagenomics)
        PIPELINE_DIR="pipelines/metagenomics"
        ;;
    sv)
...
if [ "$CONFIG_TYPE" = "simple" ]; then
    # Simple config might skip certain rules or use faster settings
    SNAKEMAKE_CMD="$SNAKEMAKE_CMD --config run_mode=simple"
fi

SNAKEMAKE_CMD="$SNAKEMAKE_CMD all"
