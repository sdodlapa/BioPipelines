"""
Usage metrics tracking for providers.
...
class UsageMetrics:
    ...
    metrics_dir: Path = field(default_factory=lambda: Path("logs/provider_metrics"))
                ...
                with open(today_file) as f:
                    ...
                    for provider_id, usage in data.get("providers", {}).items():
                        ...
                        fields = {
                            ...
                            if k not in ("avg_latency_ms",)
                        }
                        self._daily[provider_id] = ProviderUsage(**fields)
                ...
                try:
                    with open(file_path) as f:
                        ...
                        for provider_id, usage in data.get("providers", {}).items():
                            ...
                            t = totals[provider_id]
                            t.requests += usage.get("requests", 0)
                            t.tokens_used += usage.get("tokens_used", 0)
                            t.prompt_tokens += usage.get("prompt_tokens", 0)
                            t.completion_tokens += usage.get("completion_tokens", 0)
                            t.total_latency_ms += usage.get("total_latency_ms", 0)
                            t.errors += usage.get("errors", 0)
                except Exception:
                    pass
    ...
    def print_usage_report(self, days: int = 7):
        ...
        print("\n" + "=" * 60)
        print(f"PROVIDER USAGE REPORT ({days} days)")
        print("=" * 60 + "\n")
        
        for provider_id, usage in sorted(summary.get("providers", {}).items()):
