# BioPipelines: Architecture & Complete Guide

> **One Document to Rule Them All**  
> Everything you need to understand, use, and extend the BioPipelines AI Workflow Composer.

---

## Table of Contents

1. [Quick Start](#1-quick-start-2-minutes)
2. [System Overview](#2-system-overview)
3. [Request Lifecycle](#3-request-lifecycle-flowchart)
4. [Component Deep Dive](#4-component-deep-dive)
   - [4.1 Frontend (Gradio Web UI)](#41-frontend-gradio-web-ui)
   - [4.2 LLM Provider System](#42-llm-provider-system)
   - [4.3 Core Processing Pipeline](#43-core-processing-pipeline)
   - [4.4 Error Diagnosis System](#44-error-diagnosis-system)
   - [4.5 Results Visualization](#45-results-visualization)
   - [4.6 Data Discovery System](#46-data-discovery-system) ⭐
   - [4.7 Container Architecture](#47-container-architecture)
5. [Configuration](#5-configuration-reference)
6. [Troubleshooting](#6-troubleshooting)
7. [Developer Guide](#7-developer-guide)

---

## 1. Quick Start (2 Minutes)

### Prerequisites
```bash
# Conda environment (already set up on HPC)
conda activate biopipelines

# Set at least one API key
export LIGHTNING_API_KEY="your-key"  # Free 30M tokens!
# or
export OPENAI_API_KEY="your-key"
```

### Launch the Web UI
```bash
cd /home/sdodl001_odu_edu/BioPipelines
./scripts/start_gradio.sh
```

### Generate Your First Workflow
1. Open the URL shown (e.g., `http://localhost:7860`)
2. Type: `"RNA-seq differential expression for mouse, paired-end reads"`
3. Click Generate → Download → Submit to SLURM

That's it! The system handles intent parsing, tool selection, module mapping, and Nextflow generation automatically.

---

## 2. System Overview

### Philosophy

```
┌─────────────────────────────────────────────────────────────────────┐
│                    DESIGN PRINCIPLES                                 │
├─────────────────────────────────────────────────────────────────────┤
│  1. Natural Language First  - Describe what, not how               │
│  2. LLM-Agnostic            - Switch providers without code changes │
│  3. Container-Isolated      - Reproducible via Singularity         │
│  4. HPC-Native              - Built for SLURM clusters             │
│  5. Self-Healing            - Auto-diagnose and fix failures       │
│  6. Data-Connected          - Discover & download public datasets  │
└─────────────────────────────────────────────────────────────────────┘
```

### High-Level Architecture

```
┌──────────────────────────────────────────────────────────────────────────┐
│                              USER LAYER                                   │
│  ┌─────────────┐    ┌─────────────┐    ┌─────────────┐                   │
│  │  Gradio UI  │    │    CLI      │    │  Python API │                   │
│  │  (Web)      │    │ biocomposer │    │  Composer() │                   │
│  └──────┬──────┘    └──────┬──────┘    └──────┬──────┘                   │
└─────────┼──────────────────┼──────────────────┼──────────────────────────┘
          │                  │                  │
          └──────────────────┼──────────────────┘
                             ▼
┌──────────────────────────────────────────────────────────────────────────┐
│                           COMPOSER CORE                                   │
│  ┌─────────────────────────────────────────────────────────────────┐     │
│  │                         Composer                                 │     │
│  │  • Orchestrates all components                                  │     │
│  │  • Manages LLM lifecycle                                        │     │
│  │  • Handles pre-flight validation                                │     │
│  └─────────────────────────────────────────────────────────────────┘     │
│          │              │              │              │                   │
│          ▼              ▼              ▼              ▼                   │
│  ┌────────────┐  ┌────────────┐  ┌────────────┐  ┌────────────┐         │
│  │  Intent    │  │   Tool     │  │  Module    │  │ Workflow   │         │
│  │  Parser    │  │  Selector  │  │  Mapper    │  │ Generator  │         │
│  └────────────┘  └────────────┘  └────────────┘  └────────────┘         │
└──────────────────────────────────────────────────────────────────────────┘
                             │
          ┌──────────────────┼──────────────────┐
          ▼                  ▼                  ▼
┌──────────────────┐  ┌──────────────────────────────────────────────────────┐
│  DATA DISCOVERY  │  │                 LLM PROVIDER LAYER                    │
│  ┌────────────┐  │  │  ┌──────────┐  ┌──────────┐  ┌──────────┐           │
│  │   ENCODE   │  │  │  │Lightning │  │ OpenAI   │  │Anthropic │           │
│  │    GEO     │  │  │  │(Default) │  │ GPT-4    │  │ Claude   │           │
│  │    SRA     │  │  │  └──────────┘  └──────────┘  └──────────┘           │
│  │  Ensembl   │  │  │  ┌──────────┐  ┌──────────┐                         │
│  └────────────┘  │  │  │  Ollama  │  │  vLLM    │                         │
│    Find & Download │  │ │ (Local)  │  │ (GPU)    │                         │
│   Public Datasets │  │  └──────────┘  └──────────┘                         │
└──────────────────┘  └──────────────────────────────────────────────────────┘
                             │
                             ▼
┌──────────────────────────────────────────────────────────────────────────┐
│                        EXECUTION LAYER                                    │
│  ┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐       │
│  │    Nextflow     │───▶│     SLURM       │───▶│  Singularity    │       │
│  │   (Workflow)    │    │   (Scheduler)   │    │  (Containers)   │       │
│  └─────────────────┘    └─────────────────┘    └─────────────────┘       │
└──────────────────────────────────────────────────────────────────────────┘
                             │
                             ▼
┌──────────────────────────────────────────────────────────────────────────┐
│                      MONITORING & RESULTS                                 │
│  ┌────────────────┐  ┌────────────────┐  ┌────────────────┐              │
│  │ Job Monitor    │  │ Error Diagnosis│  │ Result Viewer  │              │
│  │ (Live Status)  │  │ (AI-Powered)   │  │ (Visualization)│              │
│  └────────────────┘  └────────────────┘  └────────────────┘              │
└──────────────────────────────────────────────────────────────────────────┘
```

---

## 3. Request Lifecycle Flowchart

### Complete Flow: Query → Results

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                         REQUEST LIFECYCLE                                    │
└─────────────────────────────────────────────────────────────────────────────┘

User Input: "RNA-seq differential expression for mouse, treatment vs control"
     │
     ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│ STAGE 1: INTENT PARSING                                                      │
│ ┌─────────────────────────────────────────────────────────────────────────┐ │
│ │ IntentParser.parse(query)                                               │ │
│ │                                                                         │ │
│ │  1. Send query to LLM with bioinformatics context                      │ │
│ │  2. Extract structured intent:                                          │ │
│ │     • analysis_type: "rna_seq"                                         │ │
│ │     • organism: "mus musculus"                                         │ │
│ │     • data_type: "paired_end"                                          │ │
│ │     • comparison: "treatment_vs_control"                               │ │
│ │     • confidence: 0.92                                                 │ │
│ └─────────────────────────────────────────────────────────────────────────┘ │
│ Files: src/workflow_composer/core/query_parser.py                           │
└─────────────────────────────────────────────────────────────────────────────┘
     │
     ▼ ParsedIntent
     │
┌─────────────────────────────────────────────────────────────────────────────┐
│ STAGE 2: TOOL SELECTION                                                      │
│ ┌─────────────────────────────────────────────────────────────────────────┐ │
│ │ ToolSelector.find_tools_for_analysis("rna_seq")                        │ │
│ │                                                                         │ │
│ │  1. Query tool catalog (9,909 tools)                                   │ │
│ │  2. Filter by analysis type + compatibility                            │ │
│ │  3. Return categorized tools:                                          │ │
│ │     • qc: [fastqc, multiqc, fastp]                                     │ │
│ │     • alignment: [star, hisat2]                                        │ │
│ │     • quantification: [featurecounts, salmon]                          │ │
│ │     • differential: [deseq2, edger]                                    │ │
│ └─────────────────────────────────────────────────────────────────────────┘ │
│ Files: src/workflow_composer/core/tool_selector.py                          │
│ Data:  data/tool_catalog/bioconda_tools.json                                │
└─────────────────────────────────────────────────────────────────────────────┘
     │
     ▼ Dict[category, List[Tool]]
     │
┌─────────────────────────────────────────────────────────────────────────────┐
│ STAGE 3: MODULE MAPPING                                                      │
│ ┌─────────────────────────────────────────────────────────────────────────┐ │
│ │ ModuleMapper.find_modules_for_tools(tool_names)                        │ │
│ │                                                                         │ │
│ │  1. Check existing Nextflow modules                                    │ │
│ │  2. Map tools → modules:                                               │ │
│ │     • fastqc → FASTQC (exists)                                         │ │
│ │     • star → STAR_ALIGN (exists)                                       │ │
│ │     • deseq2 → [MISSING - will auto-create]                            │ │
│ │  3. Auto-generate missing modules via LLM                              │ │
│ └─────────────────────────────────────────────────────────────────────────┘ │
│ Files: src/workflow_composer/core/module_mapper.py                          │
│ Modules: nextflow-pipelines/modules/                                        │
└─────────────────────────────────────────────────────────────────────────────┘
     │
     ▼ List[Module]
     │
┌─────────────────────────────────────────────────────────────────────────────┐
│ STAGE 4: WORKFLOW GENERATION                                                 │
│ ┌─────────────────────────────────────────────────────────────────────────┐ │
│ │ WorkflowGenerator.generate(intent, modules, llm)                       │ │
│ │                                                                         │ │
│ │  1. Select workflow template based on analysis type                    │ │
│ │  2. Assemble module imports                                            │ │
│ │  3. Generate main.nf with:                                             │ │
│ │     • Parameter definitions                                            │ │
│ │     • Channel declarations                                             │ │
│ │     • Process workflow (DAG)                                           │ │
│ │  4. Generate nextflow.config with container paths                      │ │
│ │  5. Write to output directory                                          │ │
│ └─────────────────────────────────────────────────────────────────────────┘ │
│ Files: src/workflow_composer/core/workflow_generator.py                     │
│ Templates: src/workflow_composer/templates/                                 │
│ Output: generated_workflows/<workflow_name>/                                │
└─────────────────────────────────────────────────────────────────────────────┘
     │
     ▼ Workflow object + files on disk
     │
┌─────────────────────────────────────────────────────────────────────────────┐
│ STAGE 5: EXECUTION                                                           │
│ ┌─────────────────────────────────────────────────────────────────────────┐ │
│ │ PipelineExecutor.submit_job(workflow_dir)                              │ │
│ │                                                                         │ │
│ │  1. Generate SLURM batch script                                        │ │
│ │  2. Submit via sbatch                                                  │ │
│ │  3. Nextflow orchestrates:                                             │ │
│ │     reads ─┬─▶ FASTQC ─────────────────────────────────▶ MULTIQC      │ │
│ │            │                                                ▲          │ │
│ │            └─▶ FASTP ─▶ STAR_ALIGN ─▶ FEATURECOUNTS ─▶ DESEQ2         │ │
│ │                                                                         │ │
│ │  4. Each process runs in Singularity container                         │ │
│ └─────────────────────────────────────────────────────────────────────────┘ │
│ Files: src/workflow_composer/web/gradio_app.py (PipelineExecutor)           │
│ Containers: containers/images/*.sif                                         │
└─────────────────────────────────────────────────────────────────────────────┘
     │
     ▼ Running Job
     │
┌─────────────────────────────────────────────────────────────────────────────┐
│ STAGE 6: MONITORING                                                          │
│ ┌─────────────────────────────────────────────────────────────────────────┐ │
│ │ WorkflowMonitor                                                        │ │
│ │                                                                         │ │
│ │  • Parse Nextflow logs for progress                                    │ │
│ │  • Track process completion                                            │ │
│ │  • Update UI in real-time                                              │ │
│ │  • Detect failures                                                     │ │
│ └─────────────────────────────────────────────────────────────────────────┘ │
│ Files: src/workflow_composer/monitor/                                       │
└─────────────────────────────────────────────────────────────────────────────┘
     │
     ▼ Job Completed/Failed
     │
     ├─────────────────────────────┬─────────────────────────────┐
     │ SUCCESS                     │ FAILURE                     │
     ▼                             ▼                             │
┌──────────────────────┐  ┌──────────────────────┐              │
│ STAGE 7a: RESULTS    │  │ STAGE 7b: DIAGNOSIS  │              │
│ ┌──────────────────┐ │  │ ┌──────────────────┐ │              │
│ │ ResultCollector  │ │  │ │ ErrorDiagnosis   │ │              │
│ │ • Scan outputs   │ │  │ │ Agent            │ │              │
│ │ • Categorize     │ │  │ │ • Parse logs     │ │              │
│ │ • Generate viz   │ │  │ │ • Classify error │ │              │
│ │ • Archive        │ │  │ │ • Suggest fixes  │ │              │
│ └──────────────────┘ │  │ │ • Auto-fix       │ │              │
└──────────────────────┘  │ └──────────────────┘ │              │
│ Files:                  │ Files:                │              │
│ src/.../results/        │ src/.../diagnosis/    │              │
└─────────────────────────┴───────────────────────┘              │
                                    │                            │
                                    ▼                            │
                            ┌───────────────┐                    │
                            │ Apply Fix     │────────────────────┘
                            │ Re-run with   │      (retry loop)
                            │ -resume       │
                            └───────────────┘
```

---

## 4. Component Deep Dive

### 4.1 Frontend (Gradio Web UI)

**Location:** `src/workflow_composer/web/gradio_app.py`

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                           GRADIO UI TABS                                     │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  💬 Chat          Generate workflows via natural language conversation       │
│  ├─ Provider selector (Lightning/OpenAI/Anthropic/Ollama)                   │
│  ├─ Chat history                                                            │
│  ├─ Example prompts                                                         │
│  └─ Workflow preview & download                                             │
│                                                                              │
│  🔧 Tools         Browse 9,909 bioinformatics tools                         │
│  ├─ Fuzzy search                                                            │
│  ├─ Container filter                                                        │
│  └─ Tool details                                                            │
│                                                                              │
│  📦 Modules       Explore available Nextflow modules                         │
│  ├─ Category browser                                                        │
│  └─ Module documentation                                                    │
│                                                                              │
│  🚀 Execute       Submit & monitor pipeline jobs                            │
│  ├─ Workflow selector                                                       │
│  ├─ Parameter configuration                                                 │
│  ├─ SLURM job queue                                                         │
│  └─ Live progress monitoring                                                │
│                                                                              │
│  📊 Results       Visualize pipeline outputs                                │
│  ├─ Result scanner                                                          │
│  ├─ File browser                                                            │
│  └─ Report viewer (MultiQC, etc.)                                           │
│                                                                              │
│  🔍 Diagnosis     AI-powered error analysis                                 │
│  ├─ Log analysis                                                            │
│  ├─ Root cause detection                                                    │
│  ├─ Fix suggestions                                                         │
│  └─ Auto-fix execution                                                      │
│                                                                              │
│  🗂️ Data Discovery Reference genome browser                                │
│  ├─ Search across databases                                                 │
│  └─ Download manager                                                        │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```

### 4.2 LLM Provider System

**Location:** `src/workflow_composer/llm/`

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                          LLM ADAPTER PATTERN                                 │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  LLMAdapter (base.py)          Abstract base class                          │
│       │                                                                      │
│       ├── LightningAdapter     ⚡ Lightning.ai (FREE, default)              │
│       │   └─ Model: llama-3.1-8b-instruct (30M free tokens)                 │
│       │                                                                      │
│       ├── OpenAIAdapter        🟢 OpenAI                                    │
│       │   └─ Models: gpt-4o, gpt-4-turbo, gpt-3.5-turbo                     │
│       │                                                                      │
│       ├── AnthropicAdapter     🔵 Anthropic                                 │
│       │   └─ Models: claude-3-opus, claude-3-sonnet                         │
│       │                                                                      │
│       ├── OllamaAdapter        🟠 Ollama (local)                            │
│       │   └─ Models: llama3, mistral, codellama                             │
│       │                                                                      │
│       └── vLLMAdapter          🟣 vLLM (local GPU)                          │
│           └─ Models: any HuggingFace model                                   │
│                                                                              │
│  factory.py: get_llm(provider, model) → LLMAdapter                          │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```

**Usage:**
```python
from workflow_composer.llm import get_llm

# Use Lightning.ai (free)
llm = get_llm("lightning")

# Use OpenAI GPT-4
llm = get_llm("openai", model="gpt-4o")

# Use local Ollama
llm = get_llm("ollama", model="llama3")
```

### 4.3 Core Processing Pipeline

**Location:** `src/workflow_composer/core/`

| Component | File | Purpose |
|-----------|------|---------|
| **IntentParser** | `query_parser.py` | NL → structured intent |
| **ToolSelector** | `tool_selector.py` | Intent → tool recommendations |
| **ModuleMapper** | `module_mapper.py` | Tools → Nextflow modules |
| **WorkflowGenerator** | `workflow_generator.py` | Modules → complete workflow |
| **PreflightValidator** | `preflight_validator.py` | Validate before execution |

### 4.4 Error Diagnosis System

**Location:** `src/workflow_composer/diagnosis/`

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                      ERROR DIAGNOSIS PIPELINE                                │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  LogCollector (log_collector.py)                                            │
│       │ Gather Nextflow + SLURM + process logs                              │
│       ▼                                                                      │
│  PatternMatcher (patterns.py)                                               │
│       │ 50+ regex patterns for common errors                                │
│       ▼                                                                      │
│  ErrorDiagnosisAgent (agent.py)                                             │
│       │ LLM-powered root cause analysis                                     │
│       │ Categories: RESOURCE, CONFIG, DATA, TOOL, NETWORK, etc.             │
│       ▼                                                                      │
│  AutoFixEngine (auto_fix.py)                                                │
│       │ Safe automatic fixes (chmod, mkdir, retry, etc.)                    │
│       ▼                                                                      │
│  GitHubAgent (github_agent.py)                                              │
│         Create issues for unresolved errors                                  │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```

### 4.5 Results Visualization

**Location:** `src/workflow_composer/results/`

| Component | File | Purpose |
|-----------|------|---------|
| **ResultCollector** | `collector.py` | Scan and categorize output files |
| **ResultDetector** | `detector.py` | Identify file types (QC, alignment, etc.) |
| **ResultViewer** | `viewer.py` | Generate preview content |
| **ResultArchiver** | `archiver.py` | Create downloadable archives |
| **CloudTransfer** | `cloud_transfer.py` | Upload to GCS/S3 |

### 4.6 Data Discovery System

**Location:** `src/workflow_composer/data/`

Data Discovery is a **foundational capability** that helps users find public datasets before generating workflows. It enables reproducible research by linking to ENCODE, GEO, and Ensembl data.

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                        DATA DISCOVERY ARCHITECTURE                           │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  User Query: "human liver ChIP-seq H3K27ac data"                            │
│       │                                                                      │
│       ▼                                                                      │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │ QueryParser (query_parser.py)                                        │    │
│  │   • Parse natural language to SearchQuery                           │    │
│  │   • Extract: organism, assay_type, target, tissue                   │    │
│  │   • Suggest relevant data sources                                   │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│       │                                                                      │
│       ▼ SearchQuery                                                         │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │ DataDiscovery Orchestrator (orchestrator.py)                        │    │
│  │   • Route query to appropriate adapters                             │    │
│  │   • Parallel search across sources                                  │    │
│  │   • Rank and deduplicate results                                    │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│       │                                                                      │
│       ├──────────────────┬──────────────────┬──────────────────┐            │
│       ▼                  ▼                  ▼                  ▼            │
│  ┌──────────┐      ┌──────────┐      ┌──────────┐      ┌──────────┐        │
│  │  ENCODE  │      │   GEO    │      │   SRA    │      │ Ensembl  │        │
│  │ Adapter  │      │ Adapter  │      │ Adapter  │      │ Adapter  │        │
│  │          │      │          │      │          │      │          │        │
│  │ ChIP-seq │      │ Any type │      │ Raw      │      │ Genomes  │        │
│  │ ATAC-seq │      │ Processed│      │ FASTQ    │      │ GTF/GFF  │        │
│  │ RNA-seq  │      │ + Raw    │      │ data     │      │ Indexes  │        │
│  └──────────┘      └──────────┘      └──────────┘      └──────────┘        │
│       │                  │                  │                  │            │
│       └──────────────────┴──────────────────┴──────────────────┘            │
│                                    │                                         │
│                                    ▼                                         │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │ SearchResults                                                        │    │
│  │   • List[DatasetInfo] - ranked by relevance                         │    │
│  │   • Download URLs for each dataset                                  │    │
│  │   • Metadata (organism, assay, tissue, files)                       │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│       │                                                                      │
│       ▼                                                                      │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │ Download Manager                                                     │    │
│  │   • HTTP/HTTPS/FTP downloads                                        │    │
│  │   • SRA toolkit integration (fasterq-dump)                          │    │
│  │   • Progress tracking                                               │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```

**Module Structure:**
```
src/workflow_composer/data/
├── __init__.py           # Package exports
├── downloader.py         # Basic file downloader
├── discovery/            # Data discovery system
│   ├── __init__.py       # 25+ exports
│   ├── models.py         # DataSource, SearchQuery, DatasetInfo
│   ├── query_parser.py   # NL → structured query
│   ├── orchestrator.py   # DataDiscovery main class
│   └── adapters/         # Source-specific adapters
│       ├── encode.py     # ENCODE Portal API
│       ├── geo.py        # NCBI GEO/SRA API
│       └── ensembl.py    # Ensembl REST API
└── browser/              # Gradio UI components
    └── reference_browser.py
```

**Supported Data Sources:**

| Source | ID Format | Data Types |
|--------|-----------|------------|
| **ENCODE** | ENCSR*, ENCFF* | ChIP-seq, ATAC-seq, RNA-seq, Hi-C |
| **GEO** | GSE*, GSM* | All experiment types |
| **SRA** | SRR*, SRX* | Raw FASTQ data |
| **Ensembl** | - | Reference genomes, GTF annotations |

**Usage Examples:**

```python
from workflow_composer.data import DataDiscovery, quick_search

# Initialize discovery
discovery = DataDiscovery()

# Natural language search
results = discovery.search("human liver ChIP-seq H3K27ac")

# Browse results
for dataset in results.datasets:
    print(f"{dataset.source.value}: {dataset.id}")
    print(f"  Title: {dataset.title}")
    print(f"  Organism: {dataset.organism}")
    print(f"  Files: {len(dataset.download_urls)}")
    print(f"  URL: {dataset.web_url}")

# Download a dataset
job = discovery.download(
    results.datasets[0], 
    output_dir="/data/downloads"
)

# Quick search convenience function
results = quick_search("mouse RNA-seq brain")

# Source-specific search
from workflow_composer.data import search_encode, search_geo
encode_results = search_encode("H3K4me3 human")
geo_results = search_geo("single-cell RNA-seq cancer")

# Get reference genomes
from workflow_composer.data import search_references
refs = search_references(organism="human")
```

**Integration with Workflow Generation:**

Data Discovery can be used as "Stage 0" before workflow generation:

```python
from workflow_composer import Composer
from workflow_composer.data import DataDiscovery

# 1. Find data
discovery = DataDiscovery()
results = discovery.search("human liver ChIP-seq H3K27ac ENCODE")

# 2. Download data
job = discovery.download(results.datasets[0], output_dir="./data")

# 3. Generate workflow for that data
composer = Composer()
workflow = composer.generate(
    f"ChIP-seq peak calling for {results.datasets[0].organism} "
    f"with {results.datasets[0].target} target"
)
workflow.save("./chipseq_workflow")
```

### 4.7 Container Architecture

**Location:** `containers/`

```
containers/
├── base/              # Foundation (samtools, bcftools, bwa, etc.)
│   └── base.def
├── rna-seq/           # STAR, salmon, DESeq2, edgeR
├── chip-seq/          # MACS2, HOMER, deeptools
├── atac-seq/          # Genrich, ChromVAR
├── dna-seq/           # GATK, FreeBayes, VEP
├── scrna-seq/         # Scanpy, Seurat, CellRanger
├── methylation/       # Bismark, MethylDackel
├── metagenomics/      # Kraken2, MetaPhlAn
├── hic/               # HiCExplorer, Cooler
├── long-read/         # minimap2, Flye, Medaka
├── structural-variants/ # Delly, Sniffles
└── images/            # Built .sif files (22GB total)
```

**Inheritance:**
```
base_1.0.0.sif (1.4GB)
    │
    ├── rna-seq_1.0.0.sif (+200MB)
    ├── chip-seq_1.0.0.sif (+400MB)
    ├── atac-seq_1.0.0.sif (+300MB)
    └── ... (all pipeline containers inherit from base)
```

---

## 5. Configuration Reference

### 5.1 Main Config: `config/composer.yaml`

```yaml
# LLM Provider Settings
llm:
  default_provider: lightning  # lightning, openai, anthropic, ollama, vllm
  
  lightning:
    model: llama-3.1-8b-instruct
    # API key from LIGHTNING_API_KEY env var
    
  openai:
    model: gpt-4o
    temperature: 0.7
    
  ollama:
    model: llama3
    base_url: http://localhost:11434

# Knowledge Base Paths
knowledge_base:
  tool_catalog: data/tool_catalog/bioconda_tools.json
  module_library: nextflow-pipelines/modules
  workflow_patterns: src/workflow_composer/templates

# Execution Settings
execution:
  default_profile: slurm
  container_dir: containers/images
  work_dir: /scratch/$USER/nextflow_work
```

### 5.2 SLURM Config: `config/slurm.yaml`

```yaml
default_partition: main
default_time: "48:00:00"
default_memory: "8G"

profiles:
  small:
    cpus: 4
    memory: 16G
    time: "4:00:00"
  large:
    cpus: 32
    memory: 128G
    time: "72:00:00"
  gpu:
    partition: gpu
    gres: gpu:1
```

### 5.3 Environment Variables

| Variable | Purpose | Required |
|----------|---------|----------|
| `LIGHTNING_API_KEY` | Lightning.ai API (free tier) | Recommended |
| `OPENAI_API_KEY` | OpenAI API | Optional |
| `ANTHROPIC_API_KEY` | Anthropic API | Optional |
| `GITHUB_TOKEN` | GitHub issue creation | Optional |
| `GOOGLE_CLOUD_PROJECT` | GCS uploads | Optional |

---

## 6. Troubleshooting

### Common Issues

#### "No LLM providers available"
```bash
# Check API keys
echo $LIGHTNING_API_KEY
echo $OPENAI_API_KEY

# Set Lightning.ai key (free)
export LIGHTNING_API_KEY="your-key-here"

# Or add to .secrets/lightning_key
echo "your-key" > .secrets/lightning_key
```

#### "Container not found"
```bash
# Check container exists
ls -la containers/images/

# Rebuild if needed
cd containers/rna-seq
singularity build --fakeroot rna-seq_1.0.0.sif rna-seq.def
mv rna-seq_1.0.0.sif ../images/
```

#### "Module not found for tool X"
The system auto-creates missing modules via LLM. If this fails:
```bash
# Check module library
ls nextflow-pipelines/modules/

# Manually create module
biocomposer modules --create TOOL_NAME --container rna-seq
```

#### Pipeline stuck in PENDING
```bash
# Check SLURM queue
squeue -u $USER

# Check job logs
cat logs/slurm_*.out

# Cancel and retry
scancel JOB_ID
```

#### Out of Memory Error
Edit workflow's `nextflow.config`:
```groovy
process {
    withName: 'STAR_ALIGN' {
        memory = '64 GB'
    }
}
```

### Debug Mode

```bash
# Enable verbose logging
export BIOPIPELINES_DEBUG=1

# Run with debug output
python -m workflow_composer.web.gradio_app --debug
```

---

## 7. Developer Guide

### Directory Structure

```
BioPipelines/
├── src/workflow_composer/     # Main source code
│   ├── __init__.py           # Package exports
│   ├── composer.py           # Main orchestrator
│   ├── config.py             # Configuration loader
│   ├── cli.py                # Command-line interface
│   ├── core/                 # Processing pipeline
│   ├── llm/                  # LLM adapters
│   ├── diagnosis/            # Error diagnosis
│   ├── results/              # Results visualization
│   ├── data/                 # Data discovery
│   ├── web/                  # Gradio UI
│   └── templates/            # Workflow templates
├── containers/               # Singularity definitions
├── nextflow-pipelines/       # Nextflow modules & workflows
├── config/                   # Configuration files
├── tests/                    # Test suite
└── docs/                     # Documentation
```

### Adding a New LLM Provider

1. Create adapter in `src/workflow_composer/llm/`:
```python
# my_provider_adapter.py
from .base import LLMAdapter, Message, Response

class MyProviderAdapter(LLMAdapter):
    def __init__(self, model: str = "default-model"):
        self.model = model
        self.client = MyProviderClient()
    
    def chat(self, messages: List[Message]) -> Response:
        # Convert messages and call API
        result = self.client.chat(...)
        return Response(content=result.text)
```

2. Register in `factory.py`:
```python
PROVIDERS["myprovider"] = MyProviderAdapter
```

### Adding a New Analysis Type

1. Add to `AnalysisType` enum in `core/query_parser.py`
2. Create tool mapping in `tool_selector.py`
3. Add workflow template in `templates/`
4. Create container definition in `containers/`

### Running Tests

```bash
# All tests
pytest tests/ -v

# Specific module
pytest tests/test_diagnosis.py -v

# With coverage
pytest tests/ --cov=src/workflow_composer --cov-report=html
```

---

## Quick Reference Card

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                        BIOPIPELINES QUICK REFERENCE                          │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  START UI:        ./scripts/start_gradio.sh                                 │
│  CLI GENERATE:    biocomposer generate "RNA-seq for mouse"                  │
│  CLI CHAT:        biocomposer chat --llm lightning                          │
│                                                                              │
│  PYTHON API:                                                                │
│    from workflow_composer import Composer                                   │
│    c = Composer()                                                           │
│    w = c.generate("RNA-seq for mouse")                                      │
│    w.save("my_workflow/")                                                   │
│                                                                              │
│  KEY PATHS:                                                                 │
│    Config:      config/composer.yaml                                        │
│    Modules:     nextflow-pipelines/modules/                                 │
│    Containers:  containers/images/                                          │
│    Workflows:   generated_workflows/                                        │
│    Logs:        logs/                                                       │
│                                                                              │
│  SUPPORTED ANALYSES:                                                        │
│    RNA-seq, ChIP-seq, ATAC-seq, DNA-seq, Variant Calling,                  │
│    Single-cell RNA, Metagenomics, Methylation, Hi-C, Long-read             │
│                                                                              │
│  LLM PROVIDERS:                                                             │
│    lightning (FREE), openai, anthropic, ollama (local), vllm (GPU)         │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```

---

*Document Version: 1.0 | Last Updated: November 2025*
