...
axes[0, 0].axvline(x=min_counts, color='red', linestyle='--', label=f'min={min_counts}')
axes[0, 0].axvline(x=max_counts, color='red', linestyle='--', label=f'max={max_counts}')
axes[0, 0].set_yscale('log')
axes[0, 0].legend()

axes[0, 1].hist(adata.obs['n_genes_by_counts'], bins=100, color='coral', alpha=0.7)
axes[0, 1].set_xlabel('Number of genes')
axes[0, 1].set_ylabel('Number of cells')
axes[0, 1].set_title('Distribution of detected genes')
axes[0, 1].axvline(x=min_genes, color='red', linestyle='--', label=f'min={min_genes}')
axes[0, 1].set_yscale('log')
axes[0, 1].legend()

axes[1, 0].hist(adata.obs['pct_counts_mt'], bins=100, color='mediumseagreen', alpha=0.7)
axes[1, 0].set_xlabel('Mitochondrial %')
axes[1, 0].set_ylabel('Number of cells')
axes[1, 0].set_title('Distribution of mitochondrial %')
axes[1, 0].axvline(x=max_mito, color='red', linestyle='--', label=f'max={max_mito}%')
axes[1, 0].legend()

axes[1, 1].hist(adata.obs['pct_counts_ribo'], bins=100, color='mediumpurple', alpha=0.7)
